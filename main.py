...
from dataform.dataform import FetchChatHistory
from dotenv import load_dotenv
from contextlib import asynccontextmanager
# 环境变量必须要在使用环境变量中配置前导入
load_dotenv("env_config/.env.local")
from database.mysql.mysql import mysqler
from utils.chat_history import fetch_chat_history
...
@app.get("/")
async def read_root():
    return json.dumps({"code": 0, "msg": "欢迎访问SynAIDE", "data": ""})
...
async def get_chat_history(request: FetchChatHistory):
    ...
    try:
        ...
        logger.error(f"获取用户聊天记录时出错，错误信息为: {e}")
        return json.dumps({"code": 0, "msg": "获取用户聊天记录失败", "data":  {"chat_history": []}})
